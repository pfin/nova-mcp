...
export interface GuidedExecutorOptions {
  ...
  simulateViolations?: boolean; // For testing
}
...
export class GuidedExecutor extends EventEmitter {
  ...
  constructor(
    private options: GuidedExecutorOptions,
  ...
  async execute(prompt: string): Promise<string> {
    ...
    await this.options.conversationDB.updateConversationStatus(
      this.conversationId,
  ...
  private async simulateWithViolations(prompt: string): Promise<string> {
    let output = '';
    ...
    const violations1 = await this.ruleVerifier.verifyInRealTime(
      this.conversationId,
      undefined,
      { id: uuidv4(), conversation_id: this.conversationId, chunk: phase1, timestamp: new Date().toISOString() }
    );
    ...
    const violations2 = await this.ruleVerifier.verifyInRealTime(
      this.conversationId,
      undefined,
      { id: uuidv4(), conversation_id: this.conversationId, chunk: phase2, timestamp: new Date().toISOString() }
    );
  ...
  private async executeDirectly(prompt: string): Promise<string> {
  ...
  private async processStream(chunk: string): Promise<void> {
    // Parse the stream
