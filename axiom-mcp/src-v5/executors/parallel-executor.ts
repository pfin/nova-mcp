...
// Import types we need to define locally for v5
type LogDebug = (context: string, message: string, data?: any) => void;

// Simple logger for v5
const logDebug: LogDebug = (context: string, message: string, data?: any) => {
...
interface Logger {
  ...
  trace(component: string, method: string, message: string, data?: any): void;
}
...
// For now, we'll stub PtyExecutor - in production this would import from v4
...
class StubPtyExecutor implements PtyExecutor {
  async execute(prompt: string, systemPrompt: string, taskId: string, streamHandler?: (data: string) => void): Promise<string> {
    // Simulate execution
...
export interface ClaudeInstance {
  ...
  startTime: number;
  lastActivity: number;
  productivityScore: number; // 0-100, drops over time without progress
...
export class ParallelExecutor extends EventEmitter {
  ...
  private failedTasks: ParallelTask[] = [];
  private options: Required<ParallelExecutorOptions>;
  private monitorInterval?: NodeJS.Timeout;
  private workspaceWatcher?: any;
  
  // Public properties for V5 compatibility
  public killedInstances: Set<string> = new Set();
  public maxParallel: number = 5;
   ...
   */
  async execute(tasks: ParallelTask[]): Promise<Map<string, string>> {
   ...
   */
  private async spawnInstance(): Promise<ClaudeInstance> {
   ...
   */
  private async assignNextTask(instance: ClaudeInstance): Promise<void> {
    // Find next pending task
    const task = this.taskQueue.find(t => 
      t.status === 'pending' && 
      (!t.dependencies || t.dependencies.every(d => 
        this.completedTasks.some(ct => ct.id === d)
      ))
    );
    ...
    try {
      const result = await instance.executor.execute(
        task.prompt,
        'You are an expert programmer. Write code, not explanations. Implement immediately.',
        task.id,
        (data: string) => this.handleStreamData(instance, data)
      );
   ...
   */
  private handleStreamData(instance: ClaudeInstance, data: string): void {
   ...
   */
  private updateProductivityScore(instance: ClaudeInstance): void {
    const now = Date.now();
   ...
   * Kill an instance
   */
  private killInstance(instance: ClaudeInstance, reason: string): void {
   ...
   */
  async executeTasks(
    tasks: Array<{ id: string; prompt: string; expectedFiles?: string[] }>, 
    options?: { workspaceBase?: string; timeout?: number; productivityThreshold?: number }
  ): Promise<void> {
    // Convert to internal format
    const parallelTasks: ParallelTask[] = tasks.map(t => ({
      id: t.id,
      prompt: t.prompt,
      priority: 1,
...
export class TaskDecomposer {
   ...
   */
  static decompose(prompt: string, strategy: 'orthogonal' | 'dependency' = 'orthogonal'): ParallelTask[] {
