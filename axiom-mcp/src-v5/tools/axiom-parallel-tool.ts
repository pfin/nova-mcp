...
export const axiomParallelSpawnTool = {
  ...
  async execute(args: z.infer<typeof ParallelSpawnSchema>) {
...
export const axiomParallelStatusTool = {
  ...
  async execute(args: z.infer<typeof ParallelStatusSchema>) {
    if (args.executorId) {
...
export const axiomParallelKillTool = {
  ...
  async execute(args: z.infer<typeof ParallelKillSchema>) {
    const executor = executors.get(args.executorId);
    if (!executor) {
      throw new Error(`No executor found with ID: ${args.executorId}`);
    }
 ...
 */
export function registerParallelTools(server: any) {
