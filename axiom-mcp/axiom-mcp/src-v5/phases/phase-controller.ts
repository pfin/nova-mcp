...
export class PhaseController extends EventEmitter {
  ...
  private readonly phases: Record<Phase, PhaseConfig> = {
    [Phase.RESEARCH]: {
      name: Phase.RESEARCH,
      duration: 3,
...
TIMER: 3 minutes. Think deeply, then implement.`
   ...
   */
  async executeFullCycle(initialPrompt: string): Promise<PhaseResult[]> {
    ...
    try {
      ...
      const taskPlan: TaskPlan = JSON.parse(taskPlanContent);
   ...
   */
  private async executePhase(phase: Phase, context: string): Promise<PhaseResult> {
   ...
   */
  private async executeParallelTasks(tasks: TaskPlan['tasks']): Promise<PhaseResult[]> {
   ...
   */
  private async spawnRestrictedClaude(config: PhaseConfig, context: string): Promise<ChildProcessWithoutNullStreams> {
   ...
   */
  private checkForViolations(output: string, config: PhaseConfig): void {
   ...
   * Set up phase timer
   */
  private setupPhaseTimer(minutes: number): Promise<string> {
   ...
   */
  private setupTaskTimer(minutes: number): Promise<string> {
    return new Promise((resolve) => {
   ...
   */
  private async waitForOutput(filename: string, timeoutMinutes: number): Promise<string> {
   ...
   */
  private async waitForFiles(filenames: string[], timeoutMinutes: number): Promise<string> {
   ...
   */
  private interruptProcess(processId: string): void {
   ...
   */
  private interruptAllProcesses(): void {
    for (const [id] of this.activeProcesses) {
      this.interruptProcess(id);
    }
  }
...
export function createPhaseController(workingDirectory: string): PhaseController {
