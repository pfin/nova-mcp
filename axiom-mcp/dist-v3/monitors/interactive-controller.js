...
export class InteractiveController extends EventEmitter {
     ...
     */
    registerExecutor(taskId, executor) {
     ...
     */
    injectGuidance(taskId, guidance) {
...
`;
    ...
    pauseTask(taskId, reason) {
        const executor = this.executors.get(taskId);
        if (!executor || this.pausedTasks.has(taskId))
            return;
...
`;
     ...
     */
    redirectTask(taskId, newDirection) {
        const redirect = `
...
`;
     ...
     */
    requestApproval(taskId, operation) {
     ...
     */
    handleApproval(approvalId, approved, modifications) {
     ...
     */
    provideContextualHelp(taskId, context) {
        const help = `
...
export function enhanceWithInteractiveControl(claudeSubprocess, controller) {
    const originalExecute = claudeSubprocess.execute.bind(claudeSubprocess);
    claudeSubprocess.execute = async function (prompt, options) {
