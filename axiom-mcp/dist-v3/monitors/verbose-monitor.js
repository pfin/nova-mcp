...
export class VerboseMonitor extends EventEmitter {
    tasks = new Map();
    selectedTaskId = null;
    verboseMode = 'selected';
    rl;
    isInteractive;
    eventBus;
    outputBuffer = new Map();
    maxBufferLines = 1000;
    constructor(eventBus, interactive = true) {
        super();
    ...
    handleEvent(event) {
        switch (event.event) {
            ...
            case EventType.CODE_VIOLATION:
                const violationPayload = event.payload;
     ...
     */
    shouldDisplay(taskId) {
        if (this.verboseMode === 'all')
            return true;
        if (this.verboseMode === 'selected' && taskId === this.selectedTaskId)
            return true;
    ...
    selectTask(idPrefix) {
        ...
        const task = Array.from(this.tasks.values()).find(t => t.id.startsWith(idPrefix) || t.id.substring(0, 8) === idPrefix);
        if (task) {
...
export function createVerboseMonitor(eventBus) {
    return new VerboseMonitor(eventBus);
}
//# sourceMappingURL=verbose-monitor.js.map