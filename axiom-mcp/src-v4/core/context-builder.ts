...
export interface ContextConfig {
  maxTokens: number;
  maxChunkSize: number; // For paste operations
...
export interface TaskContext {
  ...
  structure: string; // Project structure overview
  relevantPaths: string[]; // Most relevant files for this task
  tokenCount: number;
  chunks: string[]; // Pre-chunked for paste operations
}
...
export class ContextBuilder extends EventEmitter {
  private repomixCache: Map<string, RepomixOutput> = new Map();
   ...
   */
  async generateRepomixContext(projectPath: string): Promise<RepomixOutput> {
   ...
   */
  private parseRepomixXML(xml: string): RepomixOutput {
   ...
   */
  async buildDependencyGraph(repomix: RepomixOutput): Promise<void> {
   ...
   * Classify file type
   */
  private classifyFile(filePath: string): 'source' | 'test' | 'config' | 'doc' {
  ...
  async createTaskContext(
    ...
    const filesIncluded: Map<string, string> = new Map();
   ...
   */
  private selectRelevantFiles(prompt: string, repomix: RepomixOutput): string[] {
   ...
   */
  private identifyFocusAreas(prompt: string): string[] {
    const areas: string[] = [];
   ...
   */
  async createOrthogonalContexts(
   ...
   */
  private analyzeContextOverlap(contexts: Map<string, TaskContext>): void {
   ...
   */
  private rebuildFullContext(context: TaskContext): string {
    const parts: string[] = [];
