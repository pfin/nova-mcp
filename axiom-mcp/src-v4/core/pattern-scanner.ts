...
export interface PatternRule {
  ...
  action: string;
  priority: number;
  cooldown?: number; // ms before this pattern can trigger again
...
export class PatternScanner extends EventEmitter {
  ...
  startPeriodicScan(interval: number = 100) {
  ...
  getStats() {
    return {
      ...
      activeCooldowns: Array.from(this.cooldowns.entries()).filter(
        ([_, time]) => Date.now() - time < 10000
      ).length
    };
  }
}
