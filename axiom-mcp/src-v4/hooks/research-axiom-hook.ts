  ...
  handler: async (context: HookContext): Promise<HookResult> => {
    ...
    if (event === HookEvent.EXECUTION_STREAM && stream && execution?.taskId) {
      ...
      if ((context.request?.args as any)?.__research_mode) {
        ...
        const researchLimit = (context.request?.args as any).__research_limit || DEFAULT_RESEARCH_TIME_LIMIT;
        const warningTime = (context.request?.args as any).__research_warning || (researchLimit * 0.75);
