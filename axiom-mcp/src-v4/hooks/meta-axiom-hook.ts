...
let patternCache: PatternDatabase | null = null;

async function loadPatternDatabase(): Promise<PatternDatabase> {
...
async function savePatternDatabase(db: PatternDatabase): Promise<void> {
  await fs.mkdir(path.dirname(PATTERN_DB_PATH), { recursive: true });
  await fs.writeFile(PATTERN_DB_PATH, JSON.stringify(db, null, 2));
  patternCache = db;
}

function analyzeFailurePatterns(db: PatternDatabase): string[] {
...
export const metaAxiomHook: Hook = {
  ...
  handler: async (context: HookContext): Promise<HookResult> => {
    ...
    if (event === HookEvent.EXECUTION_COMPLETED && execution) {
      ...
      const executionTime = Date.now() - (execution as any).startTime || 0;
