...
export async function axiomContextBuilder(params) {
    ...
    switch (action) {
        case 'generate': {
            if (!projectPath)
                throw new Error('Project path required for generate action');
        ...
        case 'prepare': {
            if (!tasks || tasks.length === 0)
                throw new Error('Tasks required for prepare action');
        ...
        case 'optimize': {
            if (!taskId)
                throw new Error('Task ID required for optimize action');
            if (!llm)
                throw new Error('LLM type required for optimize action');
        ...
        case 'analyze': {
            if (!projectPath)
                throw new Error('Project path required for analyze action');
...
//# sourceMappingURL=axiom-context-builder.js.map