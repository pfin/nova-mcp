...
export async function axiomContextComparison(params: z.infer<typeof contextComparisonSchema>) {
  ...
  if (mode === 'clean' || mode === 'both') {
    try {
      ...
      const cleanContext = await contextBuilder.createTaskContext(
        { id: taskId, prompt },
        await contextBuilder.generateRepomixContext(projectPath || process.cwd())
      );
  ...
  if (mode === 'shadow' || mode === 'both') {
    try {
      ...
      const shadowContext = await shadowContextBuilder.glitchContext(
        { id: taskId, prompt },
        projectPath
      );
