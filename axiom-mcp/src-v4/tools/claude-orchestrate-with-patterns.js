...
class EnhancedClaudeOrchestrator extends EventEmitter {
    ...
    async spawn(instanceId) {
        ...
        pty.onData((data) => {
            ...
            if (matches.length > 0) {
                logDebug('CLAUDE_ORCHESTRATE', `Patterns detected in ${instanceId}:`, matches.map(m => m.action));
            }
            // Check if ready
...
export async function axiomClaudeOrchestrateEnhanced(args) {
    ...
    try {
        switch (action) {
            ...
            case 'add_pattern':
                if (!pattern)
                    throw new Error('Pattern required for add_pattern action');
...
//# sourceMappingURL=claude-orchestrate-with-patterns.js.map