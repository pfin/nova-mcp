...
export class ParallelExecutionObservatory extends EventEmitter {
    ...
    successTimes = [];
    // Axiom principles
    MAX_EXECUTION_TIME = 10 * 60 * 1000; // 10 minutes max
    INTERVENTION_LIMIT = 3; // Max interventions before kill
    TOXIC_PATTERN_THRESHOLD = 2; // Toxic patterns before kill
    constructor() {
        super();
    ...
    async executeRace(task) {
        ...
        try {
            const winner = await Promise.race(promises.map(p => p.then(result => result.success ? result : Promise.reject('Failed'))));
    ...
    setupInterventionHandlers(instanceId, controller) {
    ...
    shouldKill(instance) {
        ...
        const toxicCount = instance.failurePatterns.filter(p => p.includes('PLANNING') || p.includes('RESEARCH') || p.includes('false-completion')).length;
        if (toxicCount >= this.TOXIC_PATTERN_THRESHOLD)
            return true;
        // Negative success score
        if (instance.successScore < -20)
            return true;
        // No progress after multiple interventions
        if (instance.interventions > 1 && instance.successScore <= 0)
            return true;
        return false;
    }
    killInstance(instanceId, reason) {
        const instance = this.instances.get(instanceId);
        if (!instance || instance.state === 'killed')
            return;
    ...
    async waitForCompletion(instance, parentTask) {
        return new Promise((resolve) => {
...
//# sourceMappingURL=parallel-execution-observatory.js.map