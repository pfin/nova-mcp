...
export class TaskDecomposer extends EventEmitter {
    ...
    decompose(prompt) {
        ...
        if (isVague && !hasMultipleParts) {
            ...
            subtasks.push(this.createSubtask('minimal-implementation', `Create the simplest possible working implementation of: ${prompt}`, 5), this.createSubtask('robust-implementation', `Create a production-ready implementation of: ${prompt}`, 10), this.createSubtask('test-driven', `Write tests first, then implement: ${prompt}`, 8));
            return {
                id: this.generateId(),
        ...
        if (needsFile || isLarge) {
            subtasks.push(this.createSubtask('core-implementation', `Implement the core logic only. No UI, no tests, just the essential functionality for: ${prompt}`, 5));
        }
        if (needsApi) {
            subtasks.push(this.createSubtask('api-endpoints', `Create just the API endpoints/routes. Mock the business logic for: ${prompt}`, 5));
        }
        if (needsUi) {
            subtasks.push(this.createSubtask('ui-components', `Create just the UI components with mock data for: ${prompt}`, 5));
        }
        if (needsTest || isLarge) {
            subtasks.push(this.createSubtask('test-suite', `Write comprehensive tests (can be failing) for: ${prompt}`, 5));
        }
        // If no specific decomposition, create parallel approaches
        if (subtasks.length === 0) {
            subtasks.push(this.createSubtask('python-approach', `Implement in Python: ${prompt}`, 5), this.createSubtask('javascript-approach', `Implement in JavaScript: ${prompt}`, 5), this.createSubtask('minimal-approach', `Simplest possible solution: ${prompt}`, 3));
        }
        // Add integration task if multiple parts
        if (subtasks.length > 1) {
            const integrationTask = this.createSubtask('integration', `Integrate and test all components together`, 5);
    ...
    createSubtask(id, prompt, duration) {
        return {
            id,
            prompt,
            duration,
    ...
    inferExpectedOutput(prompt) {
        if (prompt.includes('test'))
            return 'test files created';
        if (prompt.includes('API'))
            return 'endpoint handlers created';
        if (prompt.includes('UI'))
            return 'component files created';
        if (prompt.includes('Python'))
            return '.py files created';
        if (prompt.includes('JavaScript'))
            return '.js files created';
    ...
    determineCriteria(prompt, subtasks) {
        return {
    ...
    validateSuccess(task, actualOutput) {
