...
export const enhancedVerboseHook = {
...
function handleExecutionStart(taskId, context) {
...
function handleStream(taskId, data, context) {
    ...
    for (const [patternName, pattern] of Object.entries(PATTERNS)) {
        ...
        if (matches) {
            ...
            switch (patternName) {
                case 'FILE_CREATED':
                    if (matches[1])
                        metrics.filesCreated.push(matches[1]);
                    break;
                case 'FILE_MODIFIED':
                    if (matches[1])
                        metrics.filesModified.push(matches[1]);
                    break;
                case 'ERROR':
                    if (matches[1])
                        metrics.errorsDetected.push(matches[1]);
                    break;
                case 'TODO':
                    if (matches[1])
                        metrics.todosDetected.push(matches[1]);
                    break;
                case 'COMMAND':
                    const cmd = matches[1] || matches[2];
                    if (cmd)
                        metrics.commandsExecuted.push(cmd);
                    break;
                case 'PYTHON_CODE':
    ...
    if (detectedPatterns.length > 0) {
        for (const pattern of detectedPatterns) {
            ...
            if (alert)
                logger.warn('EnhancedVerboseHook', 'processStream', alert);
        }
    }
...
function handleExecutionComplete(taskId, context) {
    const metrics = activeStreams.get(taskId);
    if (!metrics)
        return { action: 'continue' };
...
function handleExecutionFailed(taskId, context) {
...
function formatAlert(pattern, data) {
    const alerts = {
...
//# sourceMappingURL=enhanced-verbose-hook.js.map