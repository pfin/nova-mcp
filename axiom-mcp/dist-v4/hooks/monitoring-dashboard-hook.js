...
export const monitoringDashboardHook = {
...
function initializeTask(taskId, context) {
    const task = {
        taskId,
        startTime: Date.now(),
        lastUpdate: Date.now(),
...
function updateMetrics(taskId, data) {
    const task = activeTasks.get(taskId);
    if (!task)
        return;
    const now = Date.now();
...
function recordIntervention(taskId, context) {
    const task = activeTasks.get(taskId);
    if (!task)
        return;
...
function finalizeTask(taskId, status) {
    const task = activeTasks.get(taskId);
    if (!task)
        return;
    task.status = status;
    ...
    logger.info('MonitoringDashboard', 'finalizeTask', 'Task completed', {
        ...
        status,
        duration,
        metrics: Object.fromEntries(Object.entries(task.metrics).map(([k, v]) => [k, v instanceof Map ? Object.fromEntries(v) : v]))
    });
...
function updatePattern(task, pattern) {
...
//# sourceMappingURL=monitoring-dashboard-hook.js.map