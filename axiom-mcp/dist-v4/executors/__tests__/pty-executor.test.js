...
describe('PtyExecutor', () => {
    ...
    describe('Basic Shell Commands', () => {
        it('should execute echo command', async () => {
            ...
            const result = await executor.execute('echo "Hello from PTY"', 'test system prompt', 'test-echo-1', (data) => testOutput.push(data));
        ...
        it('should handle pwd command', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            await executor.execute('pwd', 'test system prompt', 'test-pwd-1', (data) => testOutput.push(data));
            const output = testOutput.join('');
    ...
    describe('Control Character Processing', () => {
        it('should properly process escape sequences in write()', async () => {
            ...
            const executePromise = executor.execute('cat', 'test system prompt', 'test-escape-1', (data) => testOutput.push(data));
        ...
        it('should handle \\n newlines', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const executePromise = executor.execute('cat', 'test system prompt', 'test-newline-1', (data) => testOutput.push(data));
            await new Promise(resolve => setTimeout(resolve, 500));
            // Send lines with \n
    ...
    describe('Process Control', () => {
        it('should interrupt a process with Ctrl+C', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const executePromise = executor.execute('sleep 30', 'test system prompt', 'test-interrupt-1', (data) => testOutput.push(data));
        ...
        it('should handle interrupt() method', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const executePromise = executor.execute('sleep 30', 'test system prompt', 'test-interrupt-2', (data) => testOutput.push(data));
            await new Promise(resolve => setTimeout(resolve, 500));
    ...
    describe('Claude Integration', () => {
        it('should spawn Claude and send text with proper control sequence', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const executePromise = executor.execute('claude', 'test system prompt', 'test-claude-1', (data) => testOutput.push(data));
        ...
        it('should interrupt Claude with ESC', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const executePromise = executor.execute('claude', 'test system prompt', 'test-claude-esc', (data) => testOutput.push(data));
    ...
    describe('Error Handling', () => {
        it('should handle non-existent commands', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            await executor.execute('this_command_does_not_exist_xyz123', 'test system prompt', 'test-error-1', (data) => testOutput.push(data));
            const output = testOutput.join('');
            expect(output).toContain('not found');
        });
        it('should handle shell spawn errors', async () => {
            executor = new PtyExecutor({ shell: '/non/existent/shell' });
            await expect(executor.execute('echo test', 'test system prompt', 'test-error-2', (data) => testOutput.push(data))).rejects.toThrow();
        });
    });
    describe('Stream Handling', () => {
        it('should stream output in chunks', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const chunks = [];
            await executor.execute('for i in 1 2 3; do echo "Line $i"; sleep 0.1; done', 'test system prompt', 'test-stream-1', (data) => {
                chunks.push(data);
    ...
    describe('Environment and Working Directory', () => {
        it('should use custom working directory', async () => {
            ...
            await executor.execute('pwd', 'test system prompt', 'test-cwd-1', (data) => testOutput.push(data));
            const output = testOutput.join('');
        ...
        it('should pass environment variables', async () => {
            ...
            await executor.execute('echo $TEST_VAR_XYZ', 'test system prompt', 'test-env-1', (data) => testOutput.push(data));
            const output = testOutput.join('');
    ...
    describe('Process Lifecycle', () => {
        it('should track running state correctly', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            expect(executor.isRunning()).toBe(false);
            const executePromise = executor.execute('sleep 2', 'test system prompt', 'test-lifecycle-1', (data) => testOutput.push(data));
        ...
        it('should handle kill() during execution', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            const executePromise = executor.execute('sleep 30', 'test system prompt', 'test-kill-1', (data) => testOutput.push(data));
            await new Promise(resolve => setTimeout(resolve, 500));
            expect(executor.isRunning()).toBe(true);
            // Kill the process
            executor.kill();
        ...
        it('should ignore write() after completion', async () => {
            executor = new PtyExecutor({ shell: 'bash' });
            await executor.execute('echo "Done"', 'test system prompt', 'test-write-after-1', (data) => testOutput.push(data));
            // Should not throw
            expect(() => executor.write('Too late')).not.toThrow();
        });
    });
});
//# sourceMappingURL=pty-executor.test.js.map