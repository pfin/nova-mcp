...
import nova_memory
from nova_memory.utils import setup_logging, generate_permalink
...
class NovaMemoryConfig(BaseSettings):
    """Pydantic model for Nova Memory global configuration."""
...
class ConfigManager:
    """Manages Nova Memory configuration."""
    ...
    def load_config(self) -> NovaMemoryConfig:
        """Load configuration from file or create default."""
        if self.config_file.exists():
            try:
                data = json.loads(self.config_file.read_text(encoding="utf-8"))
                return NovaMemoryConfig(**data)
            except Exception as e:  # pragma: no cover
                logger.error(f"Failed to load config: {e}")
                config = NovaMemoryConfig()
    ...
    def save_config(self, config: NovaMemoryConfig) -> None:
...
app_config: NovaMemoryConfig = config_manager.config
...
def setup_basic_memory_logging():  # pragma: no cover
    ...
    logger.info(f"Nova Memory {basic_memory.__version__} (Project: {config.project})")
    _LOGGING_SETUP = True
